...
def aadhaar_masking(input_file_path):
    try:
        ...
        masking = AadhaarMasking(yolo_model_path="yollo11m_last.pt", layout_model_tag="model_layoutlm", orientation_inference_dir="text_image_orientation_infer", classes=["Aadhar"], classes2=[""], gpu=False)
        start_step1 = time.time()
        outputfile, answers, aadhr_data = masking.run(file_path=input_file_path, output_path=OUTPUT_DIR, threshold=0.91, threshold2=0.91)
        end_step1 = time.time()
...
@app.route("/upload-file-json", methods=["POST"])
def upload_file_json():
    ...
    if 1==1: #try:
        ...
        encrypted_response = encrypt_payload(masking_result)
        
        return jsonify({"data": encrypted_response}), 200
    
    gc.collect()
    #except Exception as e:
    #    logging.error(f"Failed to decode or process file: {e}")
    #    return jsonify({"error": "Failed to decode or process file."}), 500
